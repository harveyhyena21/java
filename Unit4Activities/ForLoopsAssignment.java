 ...
 */
   ...
   * 
   * @param studentNumber Precondition: studentNumber is an int between 1 and 10 inclusive
   * @return the student name corresponding to studentNumber
   */
  public static String getStudent(int studentNumber)
  {
    String students = "Abheek Ashmit Edward Frank Grant Olivia Owen Robbie Robert Roger";

    //loops through the string and shortens it by counting spaces
    for (int i = 1; i < studentNumber; i++){
      students = students.substring(students.indexOf(" ") + 1);
    }
    
    //checks if the chosen student is the last student
    if (students.indexOf(" ") == -1) {
      return students;
    }

    //deletes the rest of the string after the chosen student's name
    students = students.substring(0, students.indexOf(" "));
    return students;
  }
  ...
  public static String getInput()
  {
    ...
    // with value between 1 and 10 inclusive.
    while (!isInteger(input) || Integer.parseInt(input) > 10 || Integer.parseInt(input) < 1)
    {
